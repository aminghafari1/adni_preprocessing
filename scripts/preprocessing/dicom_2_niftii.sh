...
elif [ "$FIELDMAP_METHOD" = "blip" ]; then 
    PA_DIR="path/to/PA"
    echo "Converting PA dicom files to nifti files... "
    ~/dcmniix/dcm2niix -z y -o "$temp_dir" "$PA_DIR"
    mv "$temp_dir"/*.nii.gz "$inputs_dir/PA.nii.gz"
    mv "$temp_dir"/*.json "$inputs_dir/PA.json"
    rm -rf "$temp_dir"/*
    AP_DIR="path/to/AP"
    echo "Converting AP dicom files to nifti files... "
    ~/dcmniix/dcm2niix -z y -o "$temp_dir" "$AP_DIR"
    mv "$temp_dir"/*.nii.gz "$inputs_dir/AP.nii.gz"
    mv "$temp_dir"/*.json "$inputs_dir/AP.json"
    rm -rf "$temp_dir"/*
else

    echo "FIELDMAP_METHODS is not set to either 'phase' or 'blip', please check your config.sh file."
fi
